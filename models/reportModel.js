...
Report.checkReport = async (report_id, is_checked) => {
  if (is_checked == 1) {
    ...
    case 1:
      ...
      break;
    case 2:
      sql = 'UPDATE user_action SET question_count = (SELECT COUNT(*) FROM questions WHERE user_id = ? AND is_bad = 0) WHERE user_id = ?';
      await pool.query(sql, [user_id, user_id]);
      break;
    case 4:
      sql = 'UPDATE user_action SET debate_count = (SELECT COUNT(*) FROM debate_history WHERE user_id = ? AND is_bad = 0) WHERE user_id = ?';
      await pool.query(sql, [user_id, user_id]);
      break;
    default:
      break;
    ...
    for (const badge_id of badge_ids) {
      ...
      case 1:
        if (
          ((badge_id === 3 && user_action.record_count < 100) || (badge_id === 4 && user_action.record_count < 1000) || 
          (badge_id === 5 && user_action.record_count < 2500) || (badge_id === 6 && user_action.record_count < 5000) || 
          (badge_id === 7 && user_action.record_count < 10000) || (badge_id === 8 && user_action.revise_count < 1) || 
          (badge_id === 9 && user_action.revise_count < 3) || (badge_id === 10 && user_action.revise_count < 10) || 
          (badge_id === 11 && user_action.revise_count < 20) || (badge_id === 27 && user_action.answer_count < 1) || 
          (badge_id === 28 && user_action.answer_count < 30) || (badge_id === 29 && user_action.answer_count < 100) || 
          (badge_id === 30 && user_action.answer_count < 200))) {
          sql = 'UPDATE badge_history SET is_bad = 1 WHERE user_id = ? AND badge_id = ?';
          await pool.query(sql, [user_id, badge_id]);
        }
        break;
      case 2:
        if ((badge_id === 24 && user_action.question_count < 1) || (badge_id === 25 && user_action.question_count < 10) || 
        ...
        break;
      case 4:
        if ((badge_id === 21 && user_action.debate_count < 1) || (badge_id === 22 && user_action.debate_count < 10) || 
