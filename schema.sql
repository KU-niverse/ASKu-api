...
ON USER_ACTION FOR EACH ROW BEGIN DECLARE 
	DECLARE DECLARE DECLARE badge_exists INT;
	SELECT
...
CREATE EVENT RESET_CONTINUOUS_ATTENDANCE_FOR_ABSENT_USERS 
	RESET_CONTINUOUS_ATTENDANCE_FOR_ABSENT_USERS RESET_CONTINUOUS_ATTENDANCE_FOR_ABSENT_USERS reset_continuous_attendance_for_absent_users ON SCHEDULE EVERY 1 DAY STARTS '2023-07-24 00:00:00'
	DO
	UPDATE `user_attend`
	SET `cont_attend` = 0
...
CREATE EVENT RESET_TODAY_ATTENDANCE_FOR_ALL_USERS 
	RESET_TODAY_ATTENDANCE_FOR_ALL_USERS RESET_TODAY_ATTENDANCE_FOR_ALL_USERS reset_today_attendance_for_all_users ON SCHEDULE EVERY 1 DAY STARTS '2023-07-24 00:00:10' -- 10초 뒤에 실행
	DO
	UPDATE `user_attend`
	SET `today_attend` = 0;
